...
Derefter gemmer laver jeg const classId og sætter dens værdi til at være id coerced til et number. Herefter har jeg et guard clause der tjekker om classId er et heltal eller nul eller mindre hvis det er bliver brugeren ledt til notfound()
...
Jeg bruger Array metoden some, der returnere true hvis der bliver fundet et element der opfylder test funktionen - hvis ikke bliver der returneret false. I test funktionen sammenligner jeg hver brugeres id med det userId der er gemt i cookies.
Denne værdi bliver gemt i variablet initialJoinedState - det bliver passet til et client komponent.
...
Da jeg gerne vil have at rating er i heltal runder jeg avgRating op og gemmer den nye værdi roundedRating. Jeg Math.ceil() metoden, der rundet et tal op til den næste integer (det vil sige at f.eks 3.01 -> 4), i modsætning til floor der runder ned af.
